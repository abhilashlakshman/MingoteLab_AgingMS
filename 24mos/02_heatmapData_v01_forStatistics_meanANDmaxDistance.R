...
org.im.det.lif <- read.delim("~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/24 mos_age/Analysed/imageDetails_2-VTA lif files.txt", header = T)
...
counted.im.det <- read.delim("~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/24 mos_age/Analysed/imageDetails_1-VTA_counted.txt", header = T)
counted.im.det$org.px.size <- org.im.det$px.size

# IMPORT THE BREGMA DETAILS FILE
pre.bregma.det <- read.delim("~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/24 mos_age/24mos_bregma.txt", header = T)
bregma <- data.frame(
...
setwd("~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/24 mos_age/1-VTA_counted tifs/output_counts/")
files <- list.files()
...
for (i in 1:length(final.out.da)) { 
  ...
  for (j in 1:length(da)) {
    ...
    if (!is.null(dim(x.daglu))) {
      # if (!is.na(x.daglu[1,1])) {
      # hist.x.da <- hist(x.da[,1], breaks = seq(0, 1600, by = 50), plot = F)
      hist.x.daglu <- hist(x.daglu[,1], breaks = seq(0, 1600, by = 50), plot = F)
...
saveRDS(out.ls.da, "DA_24mos.rds")
saveRDS(out.ls.daglu, "DAGLU_24mos.rds")


# # EXPORT THESE VALUES TO A TAB DELIMITED TEXT FILE
# write.table(max.mat.da, "~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/HeatmapData/forStats/DA_maxDistCenter_24mos.txt", row.names = T, col.names = NA, sep = "\t", quote = F)
# write.table(max.mat.daglu, "~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/HeatmapData/forStats/DAGLU_maxDistCenter_24mos.txt", row.names = T, col.names = NA, sep = "\t", quote = F)
# write.table(mean.mat.da, "~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/HeatmapData/forStats/DA_meanDistCenter_24mos.txt", row.names = T, col.names = NA, sep = "\t", quote = F)
# write.table(mean.mat.daglu, "~/OneDrive - Indiana University/ShaferLab/Others/Jackie/INTRSECT Ant.-Post._Medial-Lateral/Medial-Lateral/HeatmapData/forStats/DAGLU_meanDistCenter_24mos.txt", row.names = T, col.names = NA, sep = "\t", quote = F)
